...
(function (B, when) {
    ...
    testCase.context = {
        ...
        parse: function () {
            ...
            this.focused = FOCUSED_PREFIX.test(this.name);
...
(function (buster, sinon) {
    ...
    ba.add("callOrder", {
        assert: function (spy) {
            var agrs = buster.isArray(spy) ? spy : arguments;
            verifyFakes.apply(this, agrs);
            if (sinon.calledInOrder(agrs)) return true;

            this.expected = [].join.call(agrs, ", ");
            this.actual = sinon.orderByFirstCall(slice(agrs)).join(", ");
        },
